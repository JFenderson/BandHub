...
function generateHBCUAliases(hbcuConfig: typeof HBCU_BANDS[0]): string[] {
  ...
  // Add conference-based keywords
  if (hbcuConfig.conference) {
    const conf = hbcuConfig.conference.toLowerCase();
    // Add patterns like "swac band", "meac marching band"
    aliases.add(`${conf} ${hbcuConfig.school.split(' ')[0].toLowerCase()}`);
  }

  // Add state-based keywords
  if (hbcuConfig.state) {
    const stateAbbr = hbcuConfig.state.toLowerCase();
    const schoolFirst = hbcuConfig.school.split(' ')[0].toLowerCase();
    aliases.add(`${stateAbbr} ${schoolFirst}`);
  }

  // Add band section keywords (drumline, tubas, etc.)
  const bandSections = ['drumline', 'tubas', 'trombones', 'trumpets', 'drum majors', 'dancers'];
  const schoolShort = hbcuConfig.school.split(' ')[0].toLowerCase();
  for (const section of bandSections) {
    // This helps match videos like "Southern Drumline" or "JSU Tubas"
    aliases.add(`${schoolShort} ${section}`);
  }
